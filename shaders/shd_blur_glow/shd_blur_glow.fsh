...
void main()
{
    highp vec4 blurred_col = texture2D( gm_BaseTexture, v_vTexcoord ); //Center sample
		...
		for (offset_D1 = 1.0; offset_D1 <= (2.0 * blur_steps); offset_D1 += 2.0) {
			...
			//offset_L = mix(offset_D1, offset_D2, sample_weight_D2/(sample_weight_D1+sample_weight_D2));
