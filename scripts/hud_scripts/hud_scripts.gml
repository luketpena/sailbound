...
function hud_draw_inventory(x, y) {
	if (inventory_alpha > 0) {
		...
		for (var i=0; i<4; i++) {	
			var slot = itemInventory.slots[i];
			if (slot.active) {
				draw_sprite_ext(slot.icon, 0, pos[i].x, pos[i].y, 1, 1, 0, c_white, inventory_alpha);
			}
		}
	}
}
	...
	if (item_alpha>0) {
		...
		if (itemInventory.current != -1) {	
			...
			if (!item_singleUse) {
				...
				draw_text_color1(x+48, y, itemInventory.ammo, color, item_alpha);
			} else {
