...
if surface_exists(water_surface) {
	...
	surface_set_target(water_surface);
		...
		draw_sprite_ext(spr_pixel,0,0,settings.water_depth-1,room_width,1,0,global.c_water_horizon,1);
		//Draw the floating pattern
		draw_sprite_tiled_ext(spr_env_water_pattern,1,-pattern_pos,0,1,1,global.c_water_horizon,.25);
		draw_sprite_tiled_ext(spr_env_water_pattern,0,-pattern_pos*2,0,1,1,global.c_water_horizon,.5);
			...
			case "rm_water":
				...
				for (var i=0; i<sys_water.wave_num; i++) {
					if (sys_water.wave_active[i]) {
						draw_sprite_ext(spr_env_water_wave_front,wave_anim,sys_water.wave_x[i],0,1,1,0,global.c_water_horizon,1*global.motion);	
					}
				}
				break;
		}
		...
		for (var i=0; i<float_count; i++) {
			...
			draw_sprite_ext(o.sprite_index,0,o.x,global.water_center_y,o.image_xscale,o.image_yscale,0,global.c_water_horizon,1);
		}
