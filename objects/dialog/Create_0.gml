...
function determineIfQuestionExists(text) {
	var questionAlreadyExists = false;
	for (var i=0; i<array_length(interview.list); i++) {
		var q = interview.list[i];
		if (q.text = text) {
...
function addFollowUp(_text, _answer, _fixed = false) {
	var questionAlreadyExists = determineIfQuestionExists(_text);
	if (!questionAlreadyExists) {
		array_push(interview.list, new InterviewQuestion(_text, _answer, _fixed));
	}
}

function insertFollowUp(_text, _answer, _pos, _fixed = false) {
	var questionAlreadyExists = determineIfQuestionExists(_text);
	if (!questionAlreadyExists) {
		// Negative position offsets it from the end
		var index = (_pos >= 0) ? _pos : array_length(interview.list) + _pos;
		array_insert(interview.list, index, new InterviewQuestion(_text, _answer, _fixed));
	}
}
...
function next() {
	if (textPos < translatedLength) {
		...
		var selectedAnswer = config[conversationPos].question[select].answer;
...
function translateNext() {
	...
	if (wait.active) {
		wait.timer = config[conversationPos].wait * room_speed;
		close();	
	} else {
		takeAction();
